...
func NovoReposiotorioDeUsuarios(db *sql.DB) *Usuarios {
	return &Usuarios{db}
}
...
func (repositorio Usuarios) AtualizarUsuario(usuarioID uint64, usuario modelos.Usuario) error {
	statement, erro := repositorio.db.Prepare(
		"upadate usuarios set nome = ?, nick = ?, email = ? where id = ?",
	)
...
func (repositorio Usuarios) BuscarSeguindo(usuarioID uint64) ([]modelos.Usuario, error) {
	...
	select u.id, u.nome, u.email, u.criadoEm 
	from usuarios u inner join seguidores s
	on u.id = s.seguidor_id where s.seguidor_id = ? `, usuarioID,
	)
...
func (repositorio Usuarios) AtualizarSenha(usuarioID uint64, senha string) error {
	statement, erro := repositorio.db.Prepare(
		"upadate usuarios set senha = ? where id = ?",
	)
